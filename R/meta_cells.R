...
#' @details Unlike [aggregate_assay()], which summarizes predefined cell groups
#'   within a Seurat object, this function infers groups using PCA, a neighbor
#'   graph and community detection. Input and output matrices are cell-by-gene.
...
meta_cells <- function(
  ...
  Matrix::t(meta_cell_ge(
    matrix_raw,
    groups = membership_all,
    weights = weights,
  ...
  if (method == "coordinates") {
    if (use_nn2) {
      if (dist_method != "euclidean") {
        stop(
          "thisutils KNN is used, so ",
          dist_method,
          " distance is not acceptable.
          To use the fast neighbor search, please choose euclidean distance.
          If you want to use ",
          dist_method,
    ...
    if (use_nn2) {
      stop(
        "Fast neighbor search cannot be applied to a distance matrix.
        Please provide coordinates and set parameter from to coordinates"
      )
    }
    return(
      build_knnd(
...
build_nn2 <- function(
  ...
  knn <- thisutils::run_biocneighbors_knn(
    reference = matrix,
    k = k,
    metric = "euclidean",
    exclude_self = TRUE
  ...
  adj_list <- lapply(
    seq_len(nrow(knn$idx)),
    function(i) {
      neighbors <- knn$idx[i, ]
      neighbors[is.finite(neighbors) & !is.na(neighbors)]
    }
  )
  graph_knn <- igraph::graph_from_adj_list(
    adj_list,
