...
#' @param method Subsampling strategy.
#' @param ratio Fraction retained or aggregated.
...
subsampling <- function(
  ...
  if (!(is.numeric(ratio) && ratio > 0 && ratio <= 1)) {
    thisutils::log_message(
      "Please set {.arg ratio} value between: (0, 1]",
      message_type = "error"
    )
  }
  if (ratio >= 1) {
  ...
  matrix <- switch(
    ...
    "sample" = {
      ...
      subsample_count <- round(sample_count * ratio)
